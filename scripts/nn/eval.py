...
if __name__=='__main__':
    ...
    targetda = dssplit[TARGETVAR]
    nfieldvars = len(FIELDVARS)
    nlocalvars = len(LOCALVARS)
    nlevs = fieldsplit.shape[3]
    commonkwargs = dict(
        num_workers=WORKERS,
    ...
    for modelcfg in MODEL_CFGS:
        ...
        if currentconfig==cachedconfig:
            logger.info('   Reusing cached patch, centers, dataset, and loader')
            patch = cachedpatch
            ...
            dataset = SampleDataset(
                ...
                patch=patch,
                uselocal=uselocal)
            loader = DataLoader(dataset,batch_size=BATCHSIZE,shuffle=False,**commonkwargs)
