...
class PatchGeometry:
    ...
    def centers(self,target,lats,lons,latrange,lonrange):
        ...
        indomain = ((lats_t[latidxs]>=latrange[0])&(lats_t[latidxs]<=latrange[1])&(lons_t[lonidxs]>=lonrange[0])&(lons_t[lonidxs]<=lonrange[1]))
...
def _collate_patches(batch,dataset:PatchDataset):
    ...
    maxlevs  = geometry.maxlevs
    timelag  = geometry.timelag
...
class PatchDataLoader:
    ...
    def dataloaders(splitdata,patchconfig,uselocal,latrange,lonrange,batchsize,workers,device):
        ...
        Returns:
        - dict: dictionary with geometry, centers, datasets, loaders
        '''
