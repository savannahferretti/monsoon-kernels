...
class PatchDataset(torch.utils.data.Dataset):
    ...
    def collate(batch,dataset):
        ...
        latoffset = torch.arange(-radius,radius+1,dtype=torch.long)
        lonoffset = torch.arange(-radius,radius+1,dtype=torch.long)
        latgrid = latidx[:,None]+latoffset[None,:]
        longrid = lonidx[:,None]+lonoffset[None,:]
        plats = latgrid.shape[1]
        plons = longrid.shape[1]
        ps = dataset.ps
        lev = dataset.lev
        nlevstotal = lev.shape[0]
        latpatchidx = latgrid[:,:,None].expand(-1,-1,plons)
        lonpatchidx = longrid[:,None,:].expand(-1,plats,-1)
        if timelag>0:
